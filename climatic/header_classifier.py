...
if __name__ == '__main__': 
    ...
    train_set = [(header_features(x),y) for (x,y) in train_header]
    test_set = [(header_features(x), y) for (x,y) in test_header]
    ...
    for (header, tag) in devtest_header: 
        guess = classifier.classify(header_features(header))
        if guess != tag: 
